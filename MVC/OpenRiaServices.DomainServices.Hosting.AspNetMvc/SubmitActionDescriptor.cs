    ...
    internal sealed class SubmitActionDescriptor : ReflectedHttpActionDescriptor
    {
        ...
        public override async Task<object> ExecuteAsync(HttpControllerContext controllerContext, IDictionary<string, object> arguments, CancellationToken cancellationToken)
        {
            ...
            {*/
                ...
                var changeSet = new ChangeSet((IEnumerable<ChangeSetEntry>)arguments[ChangeSetParameterName]);
                // TODO: changeSet.SetEntityAssociations();

                var controller = (DomainController)controllerContext.Controller;
                if (!await controller.SubmitAsync(changeSet) &&
